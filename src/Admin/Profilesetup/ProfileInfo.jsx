      ...
      <div className='mt-3'>
        <form action="" className='px-2 grid gap-3 md:w-md w-full mx-auto' onSubmit={submitData}>
          <div className='grid gap-1'>
            ...
            <input type="text" className='border h-11 p-3 w-full' value={adminProfile?.fullname || ""} placeholder='Input Fullname' onChange={(e)=> setFullname(e.target.value)}/>
          </div>

          <div className='grid gap-1'>
            <label htmlFor="" className='font-bold text-md'>UserName</label>
            <input type="text" className='border h-11 p-3 w-full' value={adminProfile?.username || ""} placeholder='Input UserName' onChange={(e)=> setUsername(e.target.value)}/>
          </div>
          ...
          <div className='grid gap-1'>
            ...
            <input type="number" className='border h-11 p-3 w-full' placeholder='Input PhoneNumber' onChange={(e)=> setPhonenumber(e.target.value)} value={adminProfile?.phone_number}/>
          </div>

          <div className='absolute top-1.5 right-2'>
            <button className={`h-fit p-1 font-bold bg-blue-600/90 text-white w-20 ${!fullname || !phoneNumber ? "hidden" : ""}`} onClick={submitData}>{saveLoading ? "Updating" : "Save"}</button>
          </div>
        </form>
      </div>
